...
function epify(frase) {
  let newFrase = frase.toLowerCase();
  let fraseScomposta = newFrase.split(" ");
  if (fraseScomposta[0] === "epicode") {
    return frase;
  } else {
    fraseScomposta.unshift("EPICODE");
    let fraseModificata = fraseScomposta.join(" ");
    return fraseModificata;
  }
}
console.log(epify("come va epicode"));
...
function reverseString(frase) {
  ...
  let reverseFrase = [];
  for (let i = newFrase.length - 1; i >= 0; i--) {
    let letter = newFrase[i];
    reverseFrase.push(letter);
  }
